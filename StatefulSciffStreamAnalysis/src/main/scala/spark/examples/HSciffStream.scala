...
object HSciffStream {
  ...
  */
  def main(args: Array[String]) {
    if (args.length < 5) {
      System.err.println("Usage: SciffStream <hostname> <port> <milliseconds-batchsize> <repartitions>" +
        " <subtraceCompleteCheck[true/false]> [debug]")
      System.exit(1)
    }
    if (args.length > 5 && args(5)=="debug") DEBUG = true
    subtraceCompleteCheck = args(4).toBoolean

    val sparkConf = new SparkConf().setAppName("SciffStream")
