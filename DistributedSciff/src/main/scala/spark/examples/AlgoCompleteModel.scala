    ...
    def launchSciffServer(values: Seq[String]) : Option[ReturnObject] = {
      if (values.nonEmpty ) {
        val sciffLauncher: SciffLauncher = {
          inputFormat match{
            case InputFormat.T => new SciffLauncher(sv.model, sv.observability,
              sv.durationConstr, sv.interTaskConstr, sv.options, DEBUG,path_to_AlpBPM,path_to_swipl)
            case InputFormat.H_COMPLETE => new SciffLauncher(sv.model, sv.options, DEBUG,path_to_AlpBPM,path_to_swipl)
          }
        }
      ...
      state match {
        case None =>{
          if (values.size == 1 && (values.head == sv.lastEvent || values.head =="") )
            None  //evito di lanciare la sciff se ho ricevuto solo lastEvent per questo modello - utile probabilmente solo in AlgoPartitionedModel
          else
