 ...
 /* var rep : Int =1
  ...
  override def run(lines: ReceiverInputDStream[String], rep : Int, sv:ModelVariables, nTraces:Int=1,
    ...
    def getId(event: String) : String = {
      inputFormat match{
        case InputFormat.T => event.split("t\\(")(1).split(",")(0)
        case InputFormat.H_COMPLETE => event.split("h\\(event\\(")(1).split(",")(0)
      }
    }
    ...
    def getH(event: String, traceId:Int) : String = {
      if (event.contains(sv.lastEvent))
        ...
        inputFormat match{
          case InputFormat.T =>  "h(event("+traceId+","+event.split("event\\(")(1).dropRight(1)
          case InputFormat.H_COMPLETE => event
        }
    }
    ...
    def getActivity(h: String) : String = {
      if (h == sv.lastEvent)
        return sv.lastEvent
      inputFormat match{
        case InputFormat.T =>  h.split("h\\(event\\(")(1).split(",")(1).split("_")(0)   //with traceid inside the h atom
        case InputFormat.H_COMPLETE => {
          val temp = h.split("h\\(event\\(")(1).split(",") //with traceid and lifecycle inside the h atom
          //System.out.println("Activity: " +temp(1)+","+temp(2))
          temp(1)//+","+temp(2)
        }
      }
    ...
    def launchSciffServer(values: Seq[(Int,String)]) : Option[ReturnObject] = {
      if (values.nonEmpty ) {
        ...
        if(sm!=sv.submodelWithLastEvent) { //ridondante:launchSciffServer non dovrebbe mai esser chiamata con sm==submodelWithLastEvent
          val sciffLauncher: SciffLauncher = {
            inputFormat match{
              case InputFormat.T => new SciffLauncher(sv.submodels(sm), sv.observabilities(sm),
                sv.durationConstrs(sm), sv.interTaskConstrs(sm), sv.options, DEBUG,path_to_AlpBPM,path_to_swipl)
              case InputFormat.H_COMPLETE => new SciffLauncher(sv.submodels(sm), sv.options, DEBUG,path_to_AlpBPM,path_to_swipl)
            }
          }
          val result: Option[String] = {
            if (subtraceCompleteCheck )
              Some(sciffLauncher.launchPartitionedModel(values.map(_._2).toArray,sm, subtraceCompleteCheck, sv.subtraceCompleteIf(sm).clone(), sv.lastEvent))
            else
              Some(sciffLauncher.launchPartitionedModel(values.map(_._2).toArray, sm , subtraceCompleteCheck, Array(), sv.lastEvent))
          }
          if (result == Some("Ni")) {
