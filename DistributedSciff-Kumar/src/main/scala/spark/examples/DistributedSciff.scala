...
object DistributedSciff {
  ...
  def main(args: Array[String]) {
    ...
    if (ModelID.isModelPartitioned(model_id))
      AlgoPartitionedModel.run(lines, rep,sv,nTraces,DEBUG,subtraceCompleteCheck,AlpBPM_path,swipl_path )
    else
      AlgoCompleteModel.run(lines,rep,sv,nTraces,DEBUG,subtraceCompleteCheck,AlpBPM_path,swipl_path )
